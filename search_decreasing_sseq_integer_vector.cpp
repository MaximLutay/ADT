...
{
	int nAllocatedNew = _vector.m_nAllocated + 5; //возрастание на 5
...
void IntegerVectorAnalyze_and_Output(IntegerVector *_vector){
	...
	for (int m = 0;m < (_vector->m_nUsed);m++) {
		...
		if (n_subelements > 0 ) {
			if (_vector->m_pData + m + n_subelements >= _vector->m_pData + _vector->m_nUsed ) {  //проверка на вывод только в пределах
